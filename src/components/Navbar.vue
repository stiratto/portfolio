...
<template>
  ...
  >
    <router-link
      ...
      >stira<span class="text-[#1c1c1c] t">t</span>to</router-link
    >
    <button
      class="hamburguer"
...
.hamburguer::after {
  ...
  transition: 0.3s ease;
  pointer-events: none; /* Permite que el contenedor maneje el hover */
}
...
@media (max-width: 760px) {
  nav > ul {
    ...
    right: 0%;
    display: flex; /* Asegura que el contenido de nav-list se muestre */
...
.hamburguer.active::before {
  transform: rotate(45deg); /* La barra superior rota */
  top: -10px;
}

.hamburguer.active::after {
  width: 30px;
  transform: rotate(-45deg); /* Ejemplo de movimiento hacia arriba */
  top: 10px; /* Se mueve hacia arriba */
}
</style>
